...
//-----------------------------------------------------------------------------
u32 ArcShape::getPointCount() const
{
  return _segments + 1;
}

//-----------------------------------------------------------------------------
Vector2f ArcShape::getPoint(u32 idx) const
{
  if (idx == 0)
    return _center;
...
Color pang::ColorFromHsv(int hue, float sat, float val)
{
  hue %= 360;
  while(hue<0) hue += 360;
...
//-----------------------------------------------------------------------------
HsvColor pang::HsvFromColor(const Color& color)
{
